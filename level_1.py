...
pygame.display.set_caption("Level 1 - Galaxy Destroyers")



pygame.mixer.init()
pygame.mixer.music.load("glitch_four.mp3")
...
def init_game():
    ...
    global lore_start_time, allow_continue
...
lore_text = [
    "Tajemniczy byt znany jako GL!TCH przeniknął",
    "do kodów z przeszłości i wszystkie gry",
    "zaczęły się mutować. Mechaniki gier przestały",
    "działać, reguły się pomieszały, a postacie",
    "zachowują się odwrotnie niż powinny.",
    "Twoja misja: poznać każdą znaną Ci grę na nowo i...",
    "Przetrwać.",
    "Uważaj: każda gra, którą znasz, została...",
    "Przeprogramowana przeciwko tobie."
]
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if game_over and event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
            init_game()
            create_bunkers()
        if show_lore and allow_continue and event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
            pygame.quit()
    ...
    if show_lore:
        ...
        if lore_start_time and not allow_continue:
            elapsed = pygame.time.get_ticks() - lore_start_time
            if elapsed >= 5000:  # 15 sekund
                allow_continue = True

        if allow_continue:
            continue_text = font.render("Wciśnij ENTER by kontynuować", True, WHITE)
